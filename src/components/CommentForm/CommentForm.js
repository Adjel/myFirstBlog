...
function CommentForm({ articleId, onSubmit }) {
  ...
  const postComment = (event, articleId) => {
    event.preventDefault();
    onSubmit({ postId: articleId, name: name, email: email, body: comm });
    setEmail("");
    setName("");
    setComm("");
  };
    ...
    <Wrapper>
      <FormWrapper>
        ...
        >
          ...
          />
          <FormDivider />
          <Label htmlFor="name">Ton nom/surnom</Label>
          <FormInput
            type="name"
            name="name"
            id="name"
